...
def bars(rows):
    return pd.DataFrame(rows, columns=["open", "high", "low", "close"]).assign(
        timestamp=pd.date_range("2024-01-02T15:00Z", periods=len(rows), freq="min"), volume=10
    )
...
def test_target_penetration_and_cost_accounting():
    ...
    assert r["fees_dollars"] == 10 and r["slippage_points"] == 0.5
...
def test_contract_change_cannot_be_traded_through():
    b = bars([(100, 101, 99, 100), (100, 102, 99, 101), (105, 106, 104, 105)])
    b["contract"] = ["NQH", "NQH", "NQM"]
    assert simulate_one(b, 0, TradeSpec(1, 4, 4, 3))["status"] == "unresolved"
    assert simulate_batch(b, [0], TradeSpec(1, 4, 4, 3)).iloc[0]["status"] == "unresolved"
...
@pytest.mark.parametrize(
    "change", ["duplicate", "naive", "off_tick", "bad_ohlc", "negative_volume"]
)
...
def test_batch_matches_independent_scalar_all_exits():
    ...
    closes = 100 + np.cumsum(rng.integers(-4, 5, 400)) * 0.25
        ...
        for h in [1, 3, 10, 45]:
            ...
            for row in batch.to_dict("records"):
                ...
                for key in [
                    ...
                    assert a == c or (isinstance(a, float) and np.isnan(a) and np.isnan(c)), (
                        key,
                        a,
                        c,
                    )
