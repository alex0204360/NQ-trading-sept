...
import pandas as pd
import pytest
from nqscalp.evidence import summarize, day_evidence, acceptance, holm


def ledger(values=(2., -1., 3.)):
    return pd.DataFrame([dict(status='closed', net_points=v, net_dollars=20*v,
        holding_bars=i+1, mae_points=2., fees_dollars=10.,
...
def test_unresolved_is_unpriced_not_zero():
    t=ledger(); t.loc[1,'status']='unresolved';t.loc[1,['net_points','net_dollars']]=np.nan
    s=summarize(t, [])
...
def test_accounting_and_overlap_are_detected():
    t=ledger(); t.loc[0,'net_dollars']=41
...
def test_daily_calendar_zeros_and_timezone():
    ...
    assert e['daily_net_points']==[1.,0.]


def test_bootstrap_deterministic_and_constant_positive():
    t=ledger(tuple([2.]*80));days=list(pd.date_range('2024-01-02',periods=80).strftime('%Y-%m-%d'))
...
def test_holm_stepdown_and_invalid_values():
    assert holm({'a':.001,'b':.013,'c':.014})=={'a':True,'b':False,'c':False}
    assert holm({})=={}
    with pytest.raises(ValueError): holm({'a':float('nan')})


def test_acceptance_all_independent_gates():
    s=dict(n_closed=150,n_unresolved=0,trade_dates=40,event_weeks=12,
        net_points=75,mean_net_points=.5,stress_net_points=1,
        closed_drawdown_dollars=5000,intratrade_drawdown_dollars=5000,
        accounting_violations=0,overlap_violations=0)
    e={'lower95':.01}
