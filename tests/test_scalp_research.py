...
def test_occupancy_ends_at_actual_exit_not_fixed_45_minutes():
    rows = pd.DataFrame([
        dict(detection_index=0, exit_index=3, termination_index=3, status="closed"),
        dict(detection_index=1, exit_index=2, termination_index=2, status="closed"),
        dict(detection_index=3, exit_index=4, termination_index=4, status="closed"),
    ])
...
def test_unresolved_trade_stays_in_ledger_and_blocks_following_signal():
    rows = pd.DataFrame([
        dict(detection_index=0, exit_index=-1, termination_index=5, status="unresolved"),
        dict(detection_index=1, exit_index=2, termination_index=2, status="closed"),
    ])
...
def test_cache_detects_truncation_and_identity_change(tmp_path):
    ...
    identity = dict(source="training", code="v1", action="long")
...
def test_prototype_matching_does_not_refit_using_future_rows():
    ...
    for signature in signatures:
        np.testing.assert_array_equal(match_signature(f.iloc[:20], signature),
                                      match_signature(f, signature)[:20])
