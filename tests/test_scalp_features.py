...
def bars(n=180):
    c = 12000 + np.arange(n) * 0.25
    return pd.DataFrame(
        {
            "timestamp": pd.date_range("2024-01-03T14:00Z", periods=n, freq="min"),
            "open": c - 0.25,
            "high": c + 0.5,
            "low": c - 0.5,
            "close": c,
...
def test_validated_copy_and_nonstandard_index():
    b = bars()
    b.index = np.arange(len(b)) * 3
    v = validate_bars(b)
    pd.testing.assert_frame_equal(v, b)
    ...
    assert f.loc[b.index[60], "seq12"] == pytest.approx(3.0)
...
def test_resets_warmup(kind):
    b = bars()
    if kind == "gap":
        b.loc[80:, "timestamp"] += pd.Timedelta(minutes=1)
    else:
        b[kind] = ["a"] * 80 + ["b"] * 100
    f = compute_features(b)
...
def test_rejects_invalid_inputs(bad):
    ...
    if bad == "naive":
        ...
        b.loc[1, "timestamp"] = b.timestamp.iloc[0]
    elif bad == "reverse":
        b = b.iloc[::-1]
    elif bad == "off_tick":
        b.loc[0, "close"] += 0.1
        ...
        b.loc[0, "timestamp"] = pd.NaT
    elif bad == "missing":
        b = b.drop(columns="volume")
    ...
    schedule = pd.DataFrame(
        {
            k: [pd.Timestamp(v)]
            for k, v in {
                "market_open": "2024-01-03T13:00Z",
                "break_start": "2024-01-03T14:05Z",
                "break_end": "2024-01-03T14:07Z",
                "market_close": "2024-01-03T14:20Z",
            }.items()
        }
    )
    assert calendar_horizons(b, schedule).tolist() == [4, 3, 2, 1, 0, 0, 0, 12, 11, 10]
    assert calendar_horizons(b.iloc[:1], schedule).tolist() == [4]
    assert calendar_horizons(
        b, schedule, period_end=pd.Timestamp("2024-01-03T14:03Z")
    ).tolist() == [2, 1, 0, 0, 0, 0, 0, 0, 0, 0]
...
def test_calendar_cap_and_no_break():
    ...
    schedule = pd.DataFrame(
        {
            "market_open": [pd.Timestamp("2024-01-03T13:00Z")],
            "market_close": [pd.Timestamp("2024-01-03T18:00Z")],
        }
    )
...
def test_definitions_are_fixed_explicit_and_contextual():
    ...
    for d in ds:
        ...
        if d["session_bucket"] != "all":
            assert (f.session_bucket[hits] == d["session_bucket"]).all()
    custom = {
        "session_bucket": "all",
    ...
    with pytest.raises(ValueError):
        match_definition(f, custom)


def test_sweep_is_rejection_and_extrema_exclude_current_bar():
    b = bars()
    b.loc[100, ["open", "high", "low", "close"]] = [12024.75, 12026, 12024, 12024.5]
    f = compute_features(b)
